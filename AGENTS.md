...
- reviewのseverity、論理的な到達可能性、unit testでの再現可能性だけを理由に修正を決めません。
- 修正または修正依頼を行う前に、通常の人間操作だけで再現できるか、どの操作・条件で発生するかを確認します。
- 通常操作以外の条件が必要な場合は、その条件と現実の発生可能性を明示します。
- 発生頻度、実害、不可逆性、修正による複雑化、診断性低下、新規不具合リスクを比較して、対応するかどうかを決めます。
- 問題が実際に発生していない段階で、将来の可能性や内部的な不格好さだけを理由に修正制約へ昇格させません。
- 判断に利用者の選択が必要な場合は、確認できた事実、実際の影響、発生条件、判断が必要な理由を提示し、利用者の判断前に修正を依頼しません。
- 対応しないreview指摘も、その理由を該当review threadへ残します。
- 実装agentは、自身が実際に修正したreview指摘だけに返信し、resolveします。
- 対応しない指摘や利用者判断で修正しない指摘を、実装agentが勝手にresolveしません。
- 利用者の判断待ちのreview threadはresolveしません。
...
- 解析指示（prompt）の具体的な内容、文言、文脈、由来、変更方針、調整理由、評価観点、比較結果、過去または現在の内容を推測・復元できる情報を、PublicなIssue、PR、review thread、commit message、追跡対象のコード・コメント・ドキュメント・テスト・fixture・ログへ記録することを禁止します。
- 解析指示の変更・調整・比較を行うこと自体、その必要性や目的、具体的な作業内容をPublicなIssue、PR、review thread、commit messageへ記録することを禁止します。
- 解析指示に関する具体的な作業と検証は、利用者とのチャットおよびGitで追跡しないローカルファイルだけで扱うこと。利用者から明示的に公開を指示された場合を除き、AI agentがGitHubへ転記・要約・一般化して残すことを禁止します。
- 実装上必要な汎用的な設定名・型名・入出力の仕組みを除き、解析指示の具体的な意味や文脈を固定文字列、コメント、テスト期待値としてコードへ持ち込むことを禁止します。
