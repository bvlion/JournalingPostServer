...
- 解析指示（prompt）の具体的な内容、文言、文脈、由来、変更方針、調整理由、評価観点、比較結果、過去または現在の内容を推測・復元できる情報を、PublicなIssue、PR、review thread、commit message、追跡対象のコード・コメント・ドキュメント・テスト・fixture・ログへ記録しません。
- 解析指示の変更・調整・比較を行うこと自体、その必要性や目的、具体的な作業内容をPublicなIssue、PR、review thread、commit messageへ記録しません。
- 解析指示に関する具体的な作業と検証は、利用者とのチャットおよびGitで追跡しないローカルファイルだけで扱います。AI agentは、利用者から明示的に公開を指示された場合を除き、GitHubへ転記・要約・一般化して残しません。
- 実装上必要な汎用的な設定名・型名・入出力の仕組みを除き、解析指示の具体的な意味や文脈を固定文字列、コメント、テスト期待値としてコードへ持ち込みません。
